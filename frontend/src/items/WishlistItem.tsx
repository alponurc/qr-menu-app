...
type Props = {
  dish: DishType;
  isMostFavorited?: boolean; // ✅ Yeni prop
};

export const WishlistItem: React.FC<Props> = ({dish, isMostFavorited}) => {
  ...
  const handleFavoriteUpdate = async () => {
    try {
      const response = await addFavorite(dish.id);
      console.log('✅ Favori sayısı güncellendi:', response.favorites);
    } catch (error) {
  ...
  return (
    <li
      style={{
        ...
        borderRadius: 10,
        position: 'relative',
        border: isMostFavorited ? '2px solid gold' : 'none', // ⭐️ Öne çıkar
      }}
    >
      {isMostFavorited && (
        <div
